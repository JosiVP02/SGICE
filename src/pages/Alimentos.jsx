...
const confirm = (mensaje) =>
  ...
  const procesarConteo = async () => {
    ...
    for (const item of conteo) {
      ...
      if (diferencia !== 0) {
        await ajusteStock("alimentos", item.id, item.conteo, "Reajuste", usuarioActual);
        resultados.push({ nombre: item.nombre, anterior: item.actual, nuevo: item.conteo, diferencia });
      }
    }
    await registrarConteo("alimentos", descripcionConteo, usuarioActual);
  ...
  const cargar = async () => setProductos(await obtenerProductos("alimentos"));

  const cargarHistorial = async () => {
    try { setHistorial(await obtenerMovimientos("alimentos")); } catch (e) { console.error(e); }
  };
  ...
  const nuevo = async () => {
    ...
    await agregarProducto("alimentos", nombre, Number(nuevoCantidad || 0));
  ...
  const entrada = async () => {
    ...
    const usuarioActual = esAdmin ? "admin" : "cocina";
    await entradaStock("alimentos", Number(entradaId), Number(entradaCantidad), entradaDetalle || "Entrada manual", usuarioActual);
  ...
  const salida = async () => {
    ...
    const usuarioActual = esAdmin ? "admin" : "cocina";
    await salidaStock("alimentos", Number(salidaId), Number(salidaCantidad), salidaDetalle || "Salida manual", usuarioActual);
  ...
  const guardarEdicion = async () => {
    ...
    await editarProducto("alimentos", editando, nombre, Number(editCantidad));
    setEditando(null); await cargarHistorial(); await cargar(); toast("Producto actualizado");
  };
  ...
  const borrar = async (id) => {
    ...
    if (!ok) return;
    await eliminarProducto("alimentos", id); await cargar(); await cargarHistorial(); toast("Producto eliminado");
  };
  ...
  return (
    <div className="module-wrap">
      <div className="module-top">
        <div>
          <h1>Alimentos</h1>
          <p className="subtext">Gestión de inventario alimentario</p>
        </div>
      </div>
